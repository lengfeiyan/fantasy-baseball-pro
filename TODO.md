...
### S1：Savant 百分位排行榜接入 + statcast 评分去手调基准 ★ 推荐
- **数据**：Percentile Rankings 全联盟快照（xwOBA/xSLG/xBA/桶率/挥空率/冲刺速度等百分位）
- **价值**：FA 评分 statcast_score 改用官方百分位归一（百分位−50），永久消除
  手调"联盟典型值"的量级错配问题（xera 口径坑的根治方案）；全联盟一张快照
  替代逐人抓取，快几个量级
- **成本**：中（排行榜直链部分改版需端点调试，数据可能内嵌页面 JSON）
- **附带**：棒球追踪数据（Bat Speed，2024 起）可顺带观望，不单独排期

### S2：Savant 期望统计（xBA/xSLG/xwOBA vs 实际）——"运气指数"
- **数据**：期望统计排行榜 vs 实际统计的差值
- **价值**：Sleeper 信号翻倍（BA<xBA 买入 / BA>xBA 卖出，现有仅 xwOBA vs AVG 一个粗信号）；
  FA 推荐对状态虚热球员降权；为 F4（交易评估）提供"运气调整后真实实力"维度
- **成本**：中（与 S1 同为排行榜快照，可打包实现）
