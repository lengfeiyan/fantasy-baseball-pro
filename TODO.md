...
- **成本**：中（用户提供 CSV：赛季/队名/R/HR/RBI/SB/AB/H/W/SV/K/IP/ER/H_allow/BB_allow 每队一行；比率底数可省略用估算）

### S3：Savant 球种配置跨季对比——投手 breakout 探测器
...
- [x] F1 模拟战绩榜：core/standings.py（ProjectedStandings）——阵容各类别加总按 SGP 分母
      折算名次积分，其余 league_size−1 队按类别波动正态模拟 2000 次，输出各类别期望名次
      + 总 SGP + 总榜名次（"HR 争第 2、SB 只能第 9"）；局限已在模块 docstring 声明：
      对手阵容为统计模型，P4a 接入真实联盟数据后可替换
- [x] 接入：CLI 新增 `standings` 子命令（需已导入阵容）；GUI 分析页集成
- [x] E1 重打包：dist/FantasyBaseballPro.exe（2026-08-29 10:10 构建，含 F1 代码）
- 测试 212 → 217 passed（新增 5 个 standings 测试）

### 2026-08-27 完成（F2 表格化 + S1/S2 Savant 数据接入）
- [x] F2 GUI 表格化：可排序 Treeview 组件（数值列按数值排序/斑马纹/列头点击反转），
      排名 Top50、阵容、FA 推荐三处替换文本输出
- [x] S1 Savant 百分位排行榜：statcast_score 优先官方百分位归一（50 基准，
      分组权重打者6项/投手6项含翻转），手调基准公式自动兜底；
      双向球员两榜合并（投手行曾覆盖打者行致精英拿中性分，实测抓出）
- [x] S2 期望统计运气指数：Sleeper 信号（BA/xBA ±0.02、SLG/xSLG ±0.045、
      ERA/xERA ±0.50 正向买入负向降权）+ FA wOBA−xwOBA ±0.03 温和调权
- [x] 新增 data_fetch/savant_leaderboard.py（CSV 端点考古：正确参数为 team= 非
      position=；快照缓存 7 天）
- 测试 209 → 212 passed（新增 3 个回归测试）
